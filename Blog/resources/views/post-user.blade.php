            ...
            <div class="about-main">
                <div class="col-md-8 about-left">
                    <div class="about-one">
                        ...
                        <hr>
                        <p style="width: auto">{{ $user['email'] }}</p>
                        <p style="width: auto">{{ $user['email'] }}</p>
                    </div>
                    <hr>
                    @if(count($user->post) > 0)
                        @foreach($user->post as $post)
                            ...
                            <div class="about-two">
                                ...
                                <p>
                                    <a href="{{ route('postUser', $post['id']) }}"> {{ $user['name'] }} </a>|
                                    {{  $post['created_at'] }}|
                                    <a href="{{ route('postCategory', $post['id']) }}"> {{ $post->category['name'] }}</a>
                                </p>
                                <p>{!! $post['content']  !!}</p>
                                <div class="about-btn">
                    ...
                    @else
                        <div class="abt-left">
                            <h3  class="alert alert-info" align="center">Không có bài viết nào của người dùng này!</h3>
                        </div>
                        <div class="clear-fix"></div>
                        <hr>
                    @endif
